...
namespace Gimpo.ComputeFunctions.Computation.Operators
{
    internal readonly struct MaxOperator : IAggregationOperator
    {
        ...
        public static T GetIdentityValue<T>() where T : unmanaged, INumber<T>, IMinMaxValue<T> => T.MinValue;

        public static T Invoke<T>(T x, T y) where T : INumber<T> => T.MaxNumber(x, y);
